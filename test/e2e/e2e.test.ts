...
/**
 * In these tests we are starting botapp apps by running "npm run [path to app.js]" using ghub.io/execa.
...
describe("end-to-end-tests", () => {
  let server: any;
  let botappProcess: execa.ExecaChildProcess<string> | null;
  let botappPort: number;
  ...
  beforeEach(async () => {
    ...
    botappPort = await getPort();
  });
  afterEach(() => {
    if (server) server.close();
    if (botappProcess) botappProcess.cancel();
  });
  ...
  const runApp = async (file: string) => {
    ...
    botappProcess = execa("node", ["bin/botapp.js", "run", file], {
      env: {
        APP_ID: "1",
    ...
    // give botapp a moment to start
    await new Promise((resolve) => setTimeout(resolve, 3000));
    // the botapp process should be successfully started
    expect(botappProcess.exitCode).toBeNull();
    ...
    try {
      await fetch(`http://127.0.0.1:${botappPort}/`, {
        method: "POST",
        headers: {
      ...
      botappProcess.cancel();
      const awaitedProcess = await botappProcess;
