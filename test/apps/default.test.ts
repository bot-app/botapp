...
describe("default app", () => {
  ...
  describe("GET /botapp", () => {
    it("returns a 200 response", async () => {
      ...
      return request(server.expressApp).get("/botapp").expect(200);
    });
    ...
    describe("get info from package.json", () => {
      it("returns the correct HTML with values", async () => {
        ...
        const actual = await request(server.expressApp)
          .get("/botapp")
          .expect(200);
        expect(actual.text).toMatch("Welcome to botapp");
      ...
      it("returns the correct HTML without values", async () => {
        ...
        const actual = await request(server.expressApp)
          .get("/botapp")
          .expect(200);
        expect(actual.text).toMatch("Welcome to your Botapp App");
        expect(actual.text).toMatchSnapshot();
      });
    });
  });
  ...
  describe("GET /", () => {
    it("redirects to /botapp", async () => {
      const server = await instantiateServer(__dirname);
      await request(server.expressApp)
        .get("/")
        .expect(302)
        .expect("location", "/botapp");
    });
  });
});
