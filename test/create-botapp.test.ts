import { createBotapp, Botapp } from "../src/index.js";
...
describe("createBotapp", () => {
  test("createBotapp()", () => {
    const botapp = createBotapp({ env });
    expect(botapp).toBeInstanceOf(Botapp);
  });
  ...
  test("defaults, env", () => {
    ...
    expect(botapp.log.level).toEqual("debug");
  });
  ...
  test("defaults, overrides", () => {
    ...
    expect(botapp.log.level).toEqual("trace");
  });
  ...
  test("defaults, custom host", () => {
    ...
    expect(botapp.state.octokit.request.endpoint.DEFAULTS.baseUrl).toEqual(
