...
As you are developing your app, you will likely want to test it by repeatedly triggering the same webhook. You can simulate a webhook being delivered by saving the payload to a file, and then calling `botapp receive` from the command line.
...
```bash
$ node_modules/.bin/botapp receive -e <event-name> -p <path-to-fixture> <path-to-app>
```
...
```bash
$ node_modules/.bin/botapp receive -e pull_request -p test/fixtures/pull_request.labeled.json ./index.js
```
