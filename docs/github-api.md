...
If you want to run a Botapp App against a GitHub Enterprise instance, you'll need to create and set the `GHE_HOST` environment variable inside of the `.env` file.

```
GHE_HOST=fake.github-enterprise.com
```

When [using Botapp programmatically](/docs/development/#run-botapp-programmatically), set the `baseUrl` option for the [`Botapp`](https://botapp.github.io/api/latest/classes/botapp.Botapp.html) constructor to the full base Url of the REST API

```js
const MyBotapp = Botapp.defaults({
