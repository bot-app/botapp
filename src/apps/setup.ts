...
export const setupAppFactory = (
  ...
  async function setupApp(
    ...
    route.get("/botapp", async (req: IncomingMessage, res: ServerResponse) => {
    ...
    route.get("/", (_req, res: ServerResponse) =>
      res
        .writeHead(302, { "content-type": "text/plain", location: `/botapp` })
        .end(`Found. Redirecting to /botapp`),
    );
  };
...
function printRestartMessage(app: Botapp) {
  app.log.info("");
  app.log.info("Botapp has been set up, please restart the server!");
  app.log.info("");
}
