...
import { Logger, Botapp, BotappOctokit } from "./index.js";
...
type AdditionalOptions = {
  ...
  Octokit?: typeof BotappOctokit;
  log?: Logger;
};

/**
 *
 * @param appFnOrArgv set to either a botapp application function: `(app) => { ... }` or to process.argv
 */
export async function run(
  ...
  const botappOptions: Options = {
    appId,
    privateKey,
    redisConfig,
    secret,
    baseUrl,
    log: additionalOptions?.log || log.child({ name: "botapp" }),
  ...
  const serverOptions: ServerOptions = {
    ...
    Botapp: Botapp.defaults(botappOptions),
  };
  ...
  if (!appId || !privateKey) {
    ...
    // When botapp is started for the first time, it gets into a setup mode
    ...
    // to anything, as the Botapp instance is not used in setup it makes no
    // difference anyway.
    server = new Server({
      ...serverOptions,
  ...
  if (Array.isArray(appFnOrArgv)) {
    const pkg = await pkgConf("botapp");
