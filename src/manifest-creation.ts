...
export class ManifestCreation {
  ...
  public getManifest(pkg: PackageJson, baseUrl: string) {
    ...
    const generatedManifest = JSON.stringify({
      ...
      redirect_url: `${baseUrl}/botapp/setup`,
      // TODO: add setup url
      // setup_url:`${baseUrl}/botapp/success`,
  ...
  public async createAppFromCode(code: string, botappOptions?: OctokitOptions) {
    const octokit = new BotappOctokit(botappOptions);
