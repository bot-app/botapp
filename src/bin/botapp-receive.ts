// Usage: botapp receive -e push -p path/to/payload app.js
...
import { Botapp, type ApplicationFunctionOptions } from "../index.js";
...
async function main() {
  ...
  await botapp.load(appFn, options);

  botapp.log.debug("Receiving event", event);
  botapp.receive({ name: event, payload, id: uuidv4() }).catch(() => {
