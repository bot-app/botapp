...
import type { Botapp } from "../index.js";
...
export class Server {
  ...
  public botappApp: Botapp;

  private state: State;

  constructor(options: ServerOptions = {} as ServerOptions) {
    this.expressApp = express();
    this.botappApp = new options.Botapp({
      request: options.request,
    });
    this.log = options.log
      ? rebindLog(options.log)
      : rebindLog(this.botappApp.log.child({ name: "server" }));
    ...
    this.expressApp.use(
      createWebhooksMiddleware(this.botappApp.webhooks, {
  ...
  public async start() {
    this.log.info(
      `Running Botapp v${this.version} (Node.js: ${process.version})`,
    );
